...
import {responseCache} from './fetch';
import {useMemo} from 'react';

function writePreloadedQueryToCache<
  TRequest extends ConcreteRequest,
  TQuery extends OperationType,
>(preloadedQueryObject: SerializablePreloadedQuery<TRequest, TQuery>): void {
  const cacheKey =
...
// into QueryResponseCache, so we the network layeresponseCacher
